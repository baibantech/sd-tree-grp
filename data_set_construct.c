...
void data_pre_process()
{
	...
	void *data = NULL;
	u32 hash, hashcode, tmp, hit;

	hash_data = malloc(sizeof(u32)*(1<<pghash->used_bit));
	hash_hit = malloc(sizeof(u32)*(1<<pghash->used_bit));
	if(hash_data == 0 || hash_hit == 0)
	{
		printf("\r\n====alloc fail====\r\n");
		return;
	}
	memset(hash_data,0,(sizeof(u32)*(1<<pghash->used_bit)));
	memset(hash_hit,0,(sizeof(u32)*(1<<pghash->used_bit)));
	hash = 0;
	do {
			...
			while(data = get_next_data(next))
			{
				...
				hashcode = hash | ((1<<3)-1);
				#else
				//hashcode = hash << (32-pghash->used_bit);
				hashcode = hash << (32-17);
				hash++;
				#endif
				...
				*(u32 *)data = htonl(hashcode);
				tmp = hashcode >> (32-pghash->used_bit);
				hash_data[tmp]++;
			}
		}while(cur);
	for(tmp=0; tmp<(1<<pghash->used_bit);tmp++)
	{
		hit = hash_data[tmp];
		hash_hit[hit]++;
	}
...
//			test_insert_data(data);
			...
			if(g_mask == 1)
			{
				data = (u64 *)(data_set_config_map_address + 
                                (rand()%data_set_config_instance_num)*data_set_config_instance_len);
			}
#if 1			
            if(insert_cnt%100 == 0)
